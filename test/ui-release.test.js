// @alacris/ui publishes from starter/ with its own tag format and its own
...
const ui = JSON.parse(readFileSync(new URL('../starter/.releaserc.json', import.meta.url), 'utf8'));
const named = (name) => {
  const found = ui.plugins.find((p) => (Array.isArray(p) ? p[0] : p) === name);
  assert.ok(found, `${name} is missing from starter/.releaserc.json`);
...
test('the UI pipeline keeps only ui and starter scopes', () => {
  const filter = ui.plugins[0];
  assert.equal(filter[0], '../scripts/release-filter-scopes.js');
  assert.deepEqual(filter[1], { include: ['ui', 'starter'] });

  const kept = filterCommits(
    [commit('feat(ui): combobox'), commit('feat(html): each()'), commit('fix(starter): overlay')],
    filter[1]
  );
  assert.deepEqual(kept.map((c) => c.message), ['feat(ui): combobox', 'fix(starter): overlay']);
});
...
test('UI-scoped commits bump @alacris/ui; library commits do not', async () => {
  const bumpFiltered = async (message) => {
    const commits = filterCommits([commit(message)], { include: ['ui', 'starter'] });
  ...
  assert.equal(await bumpFiltered('fix(starter): overlay focus'), 'patch');
...
test('the UI changelog title matches starter/CHANGELOG.md', () => {
  const lf = (v) => v.split(String.fromCharCode(13)).join('');
  const title = named('@semantic-release/changelog').changelogTitle;
  const changelog = readFileSync(new URL('../starter/CHANGELOG.md', import.meta.url), 'utf8');
  assert.ok(lf(changelog).startsWith(lf(title)));
});

test('the UI npm plugin publishes the starter package, not the library', () => {
